//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "Marshal.h"

@class DTHTTPAuthMediaParam, NSDictionary, NSNumber, NSString;

@interface DTHTTPSNVideoContentModel : Marshal
{
    NSString *_picMediaId;
    NSString *_videoMediaId;
    DTHTTPAuthMediaParam *_picAuthMedia;
    DTHTTPAuthMediaParam *_videoAuthMedia;
    NSString *_fileName;
    NSString *_fileType;
    NSNumber *_fileSize;
    NSNumber *_duration;
    NSNumber *_width;
    NSNumber *_height;
    NSNumber *_bitrate;
    NSDictionary *_extension;
}

@property(copy, nonatomic) NSDictionary *extension; // @synthesize extension=_extension;
@property(retain, nonatomic) NSNumber *bitrate; // @synthesize bitrate=_bitrate;
@property(retain, nonatomic) NSNumber *height; // @synthesize height=_height;
@property(retain, nonatomic) NSNumber *width; // @synthesize width=_width;
@property(retain, nonatomic) NSNumber *duration; // @synthesize duration=_duration;
@property(retain, nonatomic) NSNumber *fileSize; // @synthesize fileSize=_fileSize;
@property(copy, nonatomic) NSString *fileType; // @synthesize fileType=_fileType;
@property(copy, nonatomic) NSString *fileName; // @synthesize fileName=_fileName;
@property(retain, nonatomic) DTHTTPAuthMediaParam *videoAuthMedia; // @synthesize videoAuthMedia=_videoAuthMedia;
@property(retain, nonatomic) DTHTTPAuthMediaParam *picAuthMedia; // @synthesize picAuthMedia=_picAuthMedia;
@property(copy, nonatomic) NSString *videoMediaId; // @synthesize videoMediaId=_videoMediaId;
@property(copy, nonatomic) NSString *picMediaId; // @synthesize picMediaId=_picMediaId;
- (void).cxx_destruct;
- (id)filedsMeta__;
- (id)init;

@end

