//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "Marshal.h"

@class NSArray, NSNumber;

@interface DTDingConfirmStatusModel : Marshal
{
    NSNumber *_dingId;
    NSArray *_uids;
    NSNumber *_dingConfirmStatus;
    NSNumber *_modelTypeInTopic;
    NSNumber *_statusChangeTime;
}

@property(retain, nonatomic) NSNumber *statusChangeTime; // @synthesize statusChangeTime=_statusChangeTime;
@property(retain, nonatomic) NSNumber *modelTypeInTopic; // @synthesize modelTypeInTopic=_modelTypeInTopic;
@property(retain, nonatomic) NSNumber *dingConfirmStatus; // @synthesize dingConfirmStatus=_dingConfirmStatus;
@property(copy, nonatomic) NSArray *uids; // @synthesize uids=_uids;
@property(retain, nonatomic) NSNumber *dingId; // @synthesize dingId=_dingId;
- (void).cxx_destruct;
- (id)filedsMeta__;
- (id)init;

@end

