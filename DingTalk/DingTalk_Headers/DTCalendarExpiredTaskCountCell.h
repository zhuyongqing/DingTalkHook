//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "DTBaseCell.h"

@class DTCalendarExpiredTaskCountItem, UIButton, UIImageView, YYLabel;

@interface DTCalendarExpiredTaskCountCell : DTBaseCell
{
    DTCalendarExpiredTaskCountItem *_item;
    UIButton *_bgButton;
    YYLabel *_titleLabel;
    UIImageView *_indicatorView;
}

@property(retain, nonatomic) UIImageView *indicatorView; // @synthesize indicatorView=_indicatorView;
@property(retain, nonatomic) YYLabel *titleLabel; // @synthesize titleLabel=_titleLabel;
@property(retain, nonatomic) UIButton *bgButton; // @synthesize bgButton=_bgButton;
@property(retain, nonatomic) DTCalendarExpiredTaskCountItem *item; // @synthesize item=_item;
- (void).cxx_destruct;
- (void)actionButtonClick;
- (void)layoutSubViewsForCell;
- (void)configCellWithItem:(id)arg1;
- (double)maxWidth;
- (id)initWithCellItem:(id)arg1;

@end

