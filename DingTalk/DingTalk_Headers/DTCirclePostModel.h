//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "OpenModel.h"

#import "DTCircleRedDotPostModelProtocol-Protocol.h"
#import "OpenModelProtocol-Protocol.h"

@class DTCircleCommentModel, DTCircleContentModel, NSArray, NSString;

@interface DTCirclePostModel : OpenModel <OpenModelProtocol, DTCircleRedDotPostModelProtocol>
{
    DTCircleContentModel *_content;
    long long _postId;
    long long _createAt;
    long long _authorUid;
    long long _authorTag;
    NSString *_authorNick;
    NSArray *_comments;
    NSArray *_userLikes;
    NSArray *_postComments;
    DTCircleCommentModel *_myLike;
    unsigned long long _status;
    NSString *_contentJson;
}

+ (void)upgradeVersion2:(id)arg1;
+ (_Bool)upgradeTableToVersion:(unsigned long long)arg1 MDHandler:(id)arg2;
+ (id)customizeTableNameWithShardingId:(id)arg1;
+ (unsigned long long)currentNewestVersion;
+ (void)createTableWithMDHandler:(id)arg1;
+ (_Bool)isShardingTable;
+ (id)tableName;
+ (id)primaryKeyName;
+ (_Bool)hasAutoIncrementPrimaryKey;
+ (_Bool)isDateInToday:(id)arg1;
+ (_Bool)isDate:(id)arg1 inSameDayAsDate:(id)arg2;
+ (id)creationDateCellInfo:(id)arg1;
+ (id)separatorCellInfoWithLastCellInfo:(id)arg1;
+ (id)cellInfoArrayForPostsWithPostModels:(id)arg1;
+ (id)cellInfoArrayForFeedWithPostModels:(id)arg1;
@property(copy, nonatomic) NSString *contentJson; // @synthesize contentJson=_contentJson;
@property(nonatomic) unsigned long long status; // @synthesize status=_status;
@property(retain, nonatomic) DTCircleCommentModel *myLike; // @synthesize myLike=_myLike;
@property(copy, nonatomic) NSArray *postComments; // @synthesize postComments=_postComments;
@property(copy, nonatomic) NSArray *userLikes; // @synthesize userLikes=_userLikes;
@property(copy, nonatomic) NSArray *comments; // @synthesize comments=_comments;
@property(copy, nonatomic) NSString *authorNick; // @synthesize authorNick=_authorNick;
@property(nonatomic) long long authorTag; // @synthesize authorTag=_authorTag;
@property(nonatomic) long long authorUid; // @synthesize authorUid=_authorUid;
@property(nonatomic) long long createAt; // @synthesize createAt=_createAt;
@property(nonatomic) long long postId; // @synthesize postId=_postId;
- (void).cxx_destruct;
- (void)updateContentJson;
@property(retain, nonatomic) DTCircleContentModel *content; // @synthesize content=_content;
- (struct UIEdgeInsets)wrapperInsets;
- (id)userLikesCommentsCellInfoArray;
- (id)feedjournalCellInfoArray;
- (id)unsupportCellInfo;
- (id)postAuthorCellInfo;
- (id)actionButtonsCellInfo;
- (id)contentCellInfoArray;
- (id)postsContentCellInfo;
- (_Bool)showsComments;
- (_Bool)showsUserLikes;
- (id)cellInfoArrayForPosts;
- (id)cellInfoArrayForFeed;
- (_Bool)isContentTypeSupported;
- (_Bool)isJournalContent;
- (_Bool)isContentDetailDisplaySupported;
- (_Bool)isCreatedToday;
- (_Bool)isLiked;
- (id)formattedStringFromCreateAt;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

