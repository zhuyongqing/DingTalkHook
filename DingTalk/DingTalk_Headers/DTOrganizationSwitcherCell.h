//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UITableViewCell.h>

@class DTBadgeView, DTImageView, DTOrganizationSwitcherModel, UILabel;

@interface DTOrganizationSwitcherCell : UITableViewCell
{
    DTOrganizationSwitcherModel *_switcherModel;
    DTImageView *_iconView;
    UILabel *_titleLabel;
    DTImageView *_checkView;
    DTBadgeView *_badgeView;
}

@property(retain, nonatomic) DTBadgeView *badgeView; // @synthesize badgeView=_badgeView;
@property(retain, nonatomic) DTImageView *checkView; // @synthesize checkView=_checkView;
@property(retain, nonatomic) UILabel *titleLabel; // @synthesize titleLabel=_titleLabel;
@property(retain, nonatomic) DTImageView *iconView; // @synthesize iconView=_iconView;
@property(retain, nonatomic) DTOrganizationSwitcherModel *switcherModel; // @synthesize switcherModel=_switcherModel;
- (void).cxx_destruct;
- (void)layoutSubviews;
- (void)configOrgSwitcherWithModel:(id)arg1 indicatorType:(unsigned long long)arg2;
- (void)configOrgSwitcherWithModel:(id)arg1 showUnreadCount:(_Bool)arg2;
- (void)setRightMargin:(double)arg1;
- (id)initWithStyle:(long long)arg1 reuseIdentifier:(id)arg2;

@end

